...
Любой метод можно вызвать выполнение HTTP GET или HTTP POST запроса. Для этого Mailru-API предоставляет соответствующие классы

* Вызов метода audio.get через HTTP GET

        GetRequest.new(api, 'audio.get', {limit: 10})

* Вызом метода audio.get через HTTP POST
       
        PostRequest.new(api, 'audio.get', {limit: 10}).post 
